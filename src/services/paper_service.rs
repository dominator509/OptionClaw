...
pub fn run_paper_once(
    ...
    let resolved_market = market_provider.snapshot(MarketDataRequest {
        contract: market_snapshot.contract.clone(),
        as_of: market_snapshot.contract.expiration,
    })?;
    let resolved_advisory = llm_advisor.advise(AdvisoryContext {
    ...
    if decision.is_accepted() {
        let _paper_report = execution_provider.execute(&intent)?;
        let _live_report = live_provider.execute_live(&intent)?;
        ...
        append_audit(&audit_path, &execution_audit)?;
    }
